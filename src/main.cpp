...
int main(int argc, char *argv[])
{
    ...
    logfault::LogManager::Instance().AddHandler(std::make_unique<logfault::StreamHandler>(std::clog, logfault::LogLevel::TRACE));

    auto settings = get_local_setting();
    if (!settings)
    {
        LFLOG_ERROR << "config file is not exist!!!" << std::endl;
        exit(1);
    }
    ...
    LFLOG_INFO << "S address:" << remote.toString() << std::endl;
    LFLOG_INFO << "---------------------" << std::endl;
    LFLOG_INFO << "L address:" << local.toString() << std::endl;

    auto key = PrivacyBase::make_sha1(keyW).value();
    auto hash = PrivacyBase::make_hmac(key, msg).value();
    LFLOG_INFO << "key:" << ToHexEX(key.begin(), key.end()) << std::endl;
    LFLOG_INFO << "hash:" << ToHexEX(hash.begin(), hash.end()) << std::endl;
    ...
    signal(SIGINT, [](int sig) -> void {
        g_p->shutdown();
        LFLOG_INFO << "signal " << sig << std::endl;
        exit(3);
    });
